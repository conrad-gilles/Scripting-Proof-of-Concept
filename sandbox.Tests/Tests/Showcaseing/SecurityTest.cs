...
public class SecurityTests
{
    ...
    public async Task WhileTrueScript()
    {
        await Assert.ThrowsExceptionAsync<ActionScriptExecutionException>(async () =>
        {
            ...
            RecentIGeneratorContext ctx = factory.CreateGeneratorContext(obj.labOrder, obj.vaccine);
            RecentActionResult ar = (RecentActionResult)await InternalScriptManager!.ExecuteScript<IActionScriptBase>("WhileTrueScript", ctx, nameof(RecentIActionScript.ExecuteAsync));
    ...
    [TestMethod]
    public void TestBasicValiCorrectTypes()
    {
        string sourceCode = """
        using System;
        using System.Threading.Tasks;
        ...
        """;
        ScriptManager.BasicValidationBeforeCompiling(sourceCode);   //shouldnt throw

        //changed first return type to V2
        sourceCode = """
        using System;
        using System.Threading.Tasks;
        using System.Collections.Generic;
        ...
        using System.Collections.Generic;
        using GeneratorScriptsGenericSimple;
        public class PediatricCondition : IConditionScript<IGeneratorContext_V4.IGeneratorContext>
        {

            public async Task<bool> EvaluateAsync(IGeneratorContext_V4.IGeneratorContext context)
        {
            if (context.Patient2.DateOfBirth.HasValue)
                {
                    var age = DateTime.Now.Year - context.Patient2.DateOfBirth.Value.Year;
